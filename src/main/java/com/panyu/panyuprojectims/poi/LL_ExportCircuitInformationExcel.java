...
public class LL_ExportCircuitInformationExcel {
	...
	public static void createLlCircuitInformationExcel(List<LlCircuitInformation> llCircuitInformationList,
        ...
        LlCircuitInformation llCircuitInformation = new LlCircuitInformation();

        // 第五步，写入实体数据 实际应用中这些数据从数据库得到，
		if (llCircuitInformationList != null) {
			// 8 从集合的最高下标 递减 添加数据
			for (int i = llCircuitInformationList.size() - 1; i >= 0; i--) {
                llCircuitInformation = llCircuitInformationList.get(i);
				//1运营商
				String operator = llCircuitInformation.getOperator();
				if (operator != null && operator.equals("null")) {
                    operator = "";
				}
                //2建设项目名称
                String buildingProject = llCircuitInformation.getBuildingProject();
                if (buildingProject != null && buildingProject.equals("null")) {
                    buildingProject = "";
                }
				//3建设年份
                String buildYear = llCircuitInformation.getBuildYear();
                if (buildYear != null && buildYear.equals("null")) {
                    buildYear = "";
				}
                //4中继段名称
                String hopName = llCircuitInformation.getHopName();
                if (hopName != null && hopName.equals("null")) {
                    hopName = "";
				}
                //5省
                String province = llCircuitInformation.getProvince();
                if(province!=null && province.equals("null")){
                    province="";
				}
                //6市
                String city = llCircuitInformation.getCity();
                if(city!=null && city.equals("null")){
                    city="";
				}
				//7区(县)
                String county = llCircuitInformation.getCounty();
                if(county!=null && county.equals("null")){
                    county="";
                }
                //8区域
                String area = llCircuitInformation.getArea();
                if(area!=null&&area.equals("null")){
                    area="";
                }
                //9所属部门
                String subordinateDepartment = llCircuitInformation.getSubordinateDepartment();
                if(subordinateDepartment!=null&&subordinateDepartment.equals("null")){
                    subordinateDepartment="";
                }
                //10公司名称
                String companyName = llCircuitInformation.getCompanyName();
                if(companyName!=null&&companyName.equals("null")){
                    companyName="";
                }
                //11设计人员
                String designPersonnel = llCircuitInformation.getDesignPersonnel();
                if(designPersonnel!=null&&designPersonnel.equals("null")){
                    designPersonnel="";
                }
                //12现场监理
                String siteLeader = llCircuitInformation.getSiteLeader();
                if(siteLeader!=null&&siteLeader.equals("null")){
                    siteLeader="";
                }
                //13专业负责人
                String professionResponsiblePerson = llCircuitInformation.getProfessionResponsiblePerson();
                if(professionResponsiblePerson!=null&&professionResponsiblePerson.equals("null")){
                    professionResponsiblePerson="";
                }
                //14区域负责人
                String districtHead = llCircuitInformation.getDistrictHead();
                if(districtHead!=null&&districtHead.equals("null")){
                    districtHead="";
                }
                //15公司名称
                String corporateName = llCircuitInformation.getCorporateName();
                if(corporateName!=null&&corporateName.equals("null")){
                    corporateName="";
                }
                //16负责人
                String personInCharge = llCircuitInformation.getPersonInCharge();
                if(personInCharge!=null&&personInCharge.equals("null")){
                    personInCharge="";
                }
                //17新建杆路
                String newPoleLine = llCircuitInformation.getNewPoleLine();
                if(newPoleLine!=null&&newPoleLine.equals("null")){
                    newPoleLine="";
                }
                //18新建管道
                String newPipe = llCircuitInformation.getNewPipe();
                if(newPipe!=null&&newPipe.equals("null")){
                    newPipe="";
                }
                //19直埋光缆
                String directBurialOpticalCable = llCircuitInformation.getDirectBurialOpticalCable();
                if(directBurialOpticalCable!=null&&directBurialOpticalCable.equals("null")){
                    directBurialOpticalCable="";
                }
                //20顶管
                String pushPipe = llCircuitInformation.getPushPipe();
                if(pushPipe!=null&&pushPipe.equals("null")){
                    pushPipe="";
                }
                //21光缆类型
                String cableType = llCircuitInformation.getCableType();
                if(cableType!=null&&cableType.equals("null")){
                    cableType="";
                }
                //22架空光缆（附挂）
                String gystsSuspended = llCircuitInformation.getGystsSuspended();
                if(gystsSuspended!=null&&gystsSuspended.equals("null")){
                    gystsSuspended="";
                }
                //23架空光缆（加挂吊线）
                String gystsSuspensionWire = llCircuitInformation.getGystsSuspensionWire();
                if(gystsSuspensionWire!=null&&gystsSuspensionWire.equals("null")){
                    gystsSuspensionWire="";
                }
                //24管道光缆
                String gysta = llCircuitInformation.getGysta();
                if(gysta!=null&&gysta.equals("null")){
                    gysta="";
                }
                //25直埋光缆
                String plowInOpticalCable = llCircuitInformation.getPlowInOpticalCable();
                if(plowInOpticalCable!=null&&plowInOpticalCable.equals("null")){
                    plowInOpticalCable="";
                }
                //26墙壁光缆
                String wallCable = llCircuitInformation.getWallCable();
                if(wallCable!=null&&wallCable.equals("null")){
                    wallCable="";
                }
                //27桥架内光缆
                String inBridgeCable = llCircuitInformation.getInBridgeCable();
                if(inBridgeCable!=null&&inBridgeCable.equals("null")){
                    inBridgeCable="";
                }
                //28路面微槽光缆
                String gyfxts = llCircuitInformation.getGyfxts();
                if(gyfxts!=null&&gyfxts.equals("null")){
                    gyfxts="";
                }
                //29数量
                String newFloorStandNumber = llCircuitInformation.getNewFloorStandNumber();
                if(newFloorStandNumber!=null&&newFloorStandNumber.equals("null")){
                    newFloorStandNumber="";
                }
                //30规格
                String newFloorStandSize = llCircuitInformation.getNewFloorStandSize();
                if(newFloorStandSize!=null&&newFloorStandSize.equals("null")){
                    newFloorStandSize="";
                }
                //31数量
                String wornFloorStandNumber = llCircuitInformation.getWornFloorStandNumber();
                if(wornFloorStandNumber!=null&&wornFloorStandNumber.equals("null")){
                    wornFloorStandNumber="";
                }
                //32规格
                String wornFloorStandSize = llCircuitInformation.getWornFloorStandSize();
                if(wornFloorStandSize!=null&&wornFloorStandSize.equals("null")){
                    wornFloorStandSize="";
                }
                //33数量
                String newWallMountedOnuNumber = llCircuitInformation.getNewWallMountedOnuNumber();
                if(newWallMountedOnuNumber!=null&&newWallMountedOnuNumber.equals("null")){
                    newWallMountedOnuNumber="";
                }
                //34规格
                String newWallMountedOnuSize = llCircuitInformation.getNewWallMountedOnuSize();
                if(newWallMountedOnuSize!=null&&newWallMountedOnuSize.equals("null")){
                    newWallMountedOnuSize="";
                }
                //35数量
                String wornWallMountedOnuNumber = llCircuitInformation.getWornWallMountedOnuNumber();
                if(wornWallMountedOnuNumber!=null&&wornWallMountedOnuNumber.equals("null")){
                    wornWallMountedOnuNumber="";
                }
                //36规格
                String wornWallMountedOnuSize = llCircuitInformation.getWornWallMountedOnuSize();
                if(wornWallMountedOnuSize!=null&&wornWallMountedOnuSize.equals("null")){
                    wornWallMountedOnuSize="";
                }
                ...
                row.createCell(8).setCellValue(subordinateDepartment);
                row.createCell(9).setCellValue(companyName);
                row.createCell(10).setCellValue(designPersonnel);
                row.createCell(11).setCellValue(siteLeader);
                row.createCell(12).setCellValue(professionResponsiblePerson);
                row.createCell(13).setCellValue(districtHead);
                row.createCell(14).setCellValue(corporateName);
                row.createCell(15).setCellValue(personInCharge);
                row.createCell(16).setCellValue(newPoleLine);
                row.createCell(17).setCellValue(newPipe);
                row.createCell(18).setCellValue(directBurialOpticalCable);
                row.createCell(19).setCellValue(pushPipe);
                row.createCell(20).setCellValue(cableType);
                row.createCell(21).setCellValue(gystsSuspended);
                row.createCell(22).setCellValue(gystsSuspensionWire);
                row.createCell(23).setCellValue(gysta);
                row.createCell(24).setCellValue(plowInOpticalCable);
                row.createCell(25).setCellValue(wallCable);
                row.createCell(26).setCellValue(inBridgeCable);
                row.createCell(27).setCellValue(gyfxts);
                row.createCell(28).setCellValue(newFloorStandNumber);
                row.createCell(29).setCellValue(newFloorStandSize);
                row.createCell(30).setCellValue(wornFloorStandNumber);
                row.createCell(31).setCellValue(wornFloorStandSize);
                row.createCell(32).setCellValue(newWallMountedOnuNumber);
                row.createCell(33).setCellValue(newWallMountedOnuSize);
                row.createCell(34).setCellValue(wornWallMountedOnuNumber);
                row.createCell(35).setCellValue(wornWallMountedOnuSize);
			}
		}
	}
	/**
